...
const assertionExecutor_1 = require("./assertionExecutor");
...
async function fillSelectLikeValue(locator, value, timeout) {
...
async function setUploadedFileFromAssetId(locator, assetId, timeout) {
...
async function executeStep(page, step, defaultTimeout = DEFAULT_TIMEOUT) {
    const timeout = step.timeout ?? defaultTimeout;
    if (assertionActions.has(step.action)) {
        await (0, assertionExecutor_1.executeAssertion)(page, step, timeout);
        return;
    }
    switch (step.action) {
        case 'goto': {
        ...
        case 'click': {
            ...
            await (await (0, selectorResolver_1.resolveStepLocator)(page, selector, 'click')).click({ timeout });
            return;
        }
        case 'hover': {
            const selector = stepSelector(step);
            ...
            const locator = await (0, selectorResolver_1.resolveStepLocator)(page, selector, 'hover');
        ...
        case 'fill': {
            ...
            const locator = await (0, selectorResolver_1.resolveStepLocator)(page, selector, 'fill');
        ...
        case 'upload': {
            ...
            const locator = await (0, selectorResolver_1.resolveStepLocator)(page, selector, 'fill');
            await setUploadedFileFromAssetId(locator, step.value, timeout);
            return;
        }
        case 'press': {
            const selector = stepSelector(step);
            ...
            await (await (0, selectorResolver_1.resolveStepLocator)(page, selector, 'press')).press(step.value ?? 'Enter', { timeout });
            return;
        }
        case 'waitFor': {
            const selector = stepSelector(step);
            ...
            await (await (0, selectorResolver_1.resolveStepLocator)(page, selector, 'waitFor')).waitFor({ state: 'visible', timeout });
            return;
        }
        default:
