...
exports.parseSelectorHint = parseSelectorHint;
exports.resolveStepLocator = resolveStepLocator;
...
function looksLikeSelectorExpression(value) {
...
function buildDynamicIdFallbackCandidates(page, selector) {
...
function parseSelectorHint(value) {
...
function buildSemanticSelectors(hint) {
...
function buildClickSemanticSelectors(hint) {
    const lowerHint = hint.toLowerCase();
...
function expandMeaningTokens(hint) {
    ...
    if (normalized === 'sang')
        tokens.add('morning');
    if (normalized === 'chieu')
        tokens.add('afternoon');
    if (normalized === 'toi')
        tokens.add('evening');
    return Array.from(tokens).filter(Boolean);
}
function buildMeaningAttributeCandidates(page, hint) {
    const tokens = expandMeaningTokens(hint);
    const selectors = [];
    for (const token of tokens) {
        const escapedToken = escapeForAttribute(token);
        selectors.push(`[data-testid="${escapedToken}"]`, `[data-testid*="${escapedToken}" i]`, `[data-value="${escapedToken}"]`, `[value="${escapedToken}"]`, `[aria-label="${escapedToken}"]`, `[title="${escapedToken}"]`, `[name="${escapedToken}"]`, `[id*="${escapedToken}" i]`, `[class*="${escapedToken}" i]`, `[data-state*="${escapedToken}" i]`);
    }
    return selectors.map((selector) => page.locator(selector));
}
function buildGenericClickTextCandidates(page, hintRegex, exactHint) {
    ...
    return [
        ...
        page.locator(`xpath=//*[normalize-space(.)="${escapedXPathText}"]/ancestor-or-self::*[
        self::button or self::a or @role="button" or @role="tab" or @onclick or contains(@class,"cursor-pointer")
      ][1]`),
        page.locator(`xpath=//*[contains(normalize-space(.), "${escapedXPathText}")]/ancestor-or-self::*[
        self::button or self::a or @role="button" or @role="tab" or @onclick or contains(@class,"cursor-pointer")
      ][1]`),
        page.locator(`xpath=//*[normalize-space(.)="${escapedXPathText}"]`),
        page.locator(`xpath=//*[contains(normalize-space(.), "${escapedXPathText}")]`),
    ];
}
function buildFillCandidates(page, hintRegex, attributeHint, semanticLocators, exactHint) {
    const inputTags = 'input, textarea, select';
    const interactiveInputXPath = 'self::input[not(@type="hidden") and not(@aria-hidden="true") and not(@tabindex="-1")] or self::textarea[not(@aria-hidden="true") and not(@tabindex="-1")] or self::select[not(@aria-hidden="true") and not(@tabindex="-1")]';
...
function candidateLocators(page, hint, intent) {
...
async function pickFirstExistingLocator(candidates) {
...
async function resolveInteractableCandidate(candidates, timeoutMs) {
...
async function waitForExistingLocator(candidates, timeoutMs) {
    const started = Date.now();
    while (Date.now() - started < timeoutMs) {
...
async function resolveStepLocator(page, hint, intent) {
    const parsedHint = parseSelectorHint(hint);
    ...
    throw new Error(`Could not find element for "${parsedHint.value}". Choose an element type like Text, Button, Label, Placeholder, or CSS and try again.`);
}
