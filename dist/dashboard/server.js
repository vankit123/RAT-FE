...
function startJsonStream(response) {
...
function writeJsonStream(response, payload) {
...
function resolveSafePath(rootDir, requestPath) {
    ...
    const absolutePath = path_1.default.normalize(path_1.default.join(rootDir, relativePath));
...
async function proxyBackendJson(response, method, pathName, body) {
...
async function checkReachableUrl(rawUrl) {
...
async function postBackendJson(pathName, body, baseUrl = backendBaseUrl) {
...
function actualValueForStep(step, status) {
    if (step.action === 'assertVisible')
        return status === 'passed' ? 'visible' : 'not_found';
    if (step.action === 'assertText')
        return status === 'passed' ? step.value || null : 'not_matched';
    if (step.action === 'assertUrlContains')
        return status === 'passed' ? step.value || null : 'not_matched';
    return null;
}
async function saveBackendRunResult(flow, result, baseUrl) {
    if (!flow.backend)
        return;
    const dataSetIds = flow.backend.testDataSetIds;
    const testRun = await postBackendJson('/test-runs', {
    ...
    for (const [testDataSetId, items] of stepsByDataSet.entries()) {
        ...
        const testRunCase = await postBackendJson('/test-run-cases', {
...
const server = http_1.default.createServer(async (request, response) => {
    ...
    const requestUrl = new url_1.URL(request.url || '/', `http://${request.headers.host || `${host}:${port}`}`);
    ...
    if (method === 'GET' && requestUrl.pathname === '/api/templates') {
        sendJson(response, 200, { templates: (0, templateLibrary_1.getTemplateSummaries)() });
        return;
    }
    ...
    if (method === 'POST' && requestUrl.pathname === '/api/run-template') {
        try {
            const payload = await readRequestBody(request);
            const flow = (0, templateLibrary_1.createFlowFromTemplate)(payload.templateId, payload.input);
            const result = await (0, flowRunner_1.runFlow)({ prefix: payload.templateId || 'template', flow, input: payload.input });
    ...
    if (method === 'POST' && requestUrl.pathname === '/api/run-custom') {
        try {
            const payload = await readRequestBody(request);
            const result = await (0, flowRunner_1.runFlow)({
                prefix: 'custom',
                flow: {
    ...
    if (method === 'POST' && requestUrl.pathname === '/api/run-backend-test-case') {
        try {
            ...
            if (payload.stream) {
                ...
                try {
                    const flow = await (0, backendFlowLoader_1.loadBackendTestCaseFlow)(testCaseId, selectedBackendBaseUrl);
                    const result = await (0, flowRunner_1.runFlow)({
    ...
    if (method === 'POST' && requestUrl.pathname === '/api/recordings/start') {
        try {
            const payload = await readRequestBody(request);
            sendJson(response, 200, await (0, recorder_1.startRecording)(payload));
        }
        catch (error) {
            sendJson(response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
        }
    ...
    if (method === 'GET' && requestUrl.pathname.startsWith('/api/recordings/')) {
        try {
            ...
            sendJson(response, 200, await (0, recorder_1.getRecordingSnapshot)(sessionId));
        }
        catch (error) {
    ...
    if (method === 'POST' && requestUrl.pathname === '/api/recordings/stop') {
        try {
            const payload = await readRequestBody(request);
            sendJson(response, 200, await (0, recorder_1.stopRecording)(payload.sessionId));
        }
        catch (error) {
            sendJson(response, 400, { ok: false, error: error instanceof Error ? error.message : String(error) });
        }
    ...
    if (method === 'GET' && requestUrl.pathname.startsWith('/artifacts/')) {
        ...
        if (!artifactPath || !fs_1.default.existsSync(artifactPath) || fs_1.default.statSync(artifactPath).isDirectory()) {
    ...
    if (method === 'GET' && requestUrl.pathname.endsWith('.js')) {
        ...
        if (!filePath || !fs_1.default.existsSync(filePath) || fs_1.default.statSync(filePath).isDirectory()) {
